...
export async function POST(req: Request) {
  try {
    const { hash, selectedBumpIds, customerData, trackingData } = await req.json();
    ...
    // 3. Create Session in CajuPay
    const idempotencyKey = `${finalCheckout.id}-${Date.now()}`;
    const cajuRes = await fetch("https://api.cajupay.com.br/api/sdk/v1/checkout/sessions", {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        ...
        allow_pix: false, // PIX is handled separately via API
      }),
    });

    if (!cajuRes.ok) {
      const errorText = await cajuRes.text();
      console.error("[CAJUPAY SESSION ERROR]", errorText);
      return NextResponse.json({ error: "Erro ao criar sessão no provedor de pagamento" }, { status: 500 });
    }

    const cajuSession = await cajuRes.json();
    ...
    const mainSaleId = `${cajuSession.checkout_session_id}`;

    const salesToInsert = [
      {
        user_id: userId,
        product_id: finalProduct.id,
        stripe_payment_intent_id: mainSaleId, // We use this column to store the cajupay session id
    ...
    for (const bump of orderbumps) {
      ...
      salesToInsert.push({
        ...
        stripe_payment_intent_id: mainSaleId,
        amount: bumpPrice,
    ...
    await supabaseAdmin.from("sales").delete().eq("stripe_payment_intent_id", mainSaleId);
    ...
    return NextResponse.json({
      checkout_session_id: cajuSession.checkout_session_id,
      token: cajuSession.token,
    });
